...
def test_refresh_token_roundtrip() -> None:
    settings = _settings()
    token, token_id = create_refresh_token(user_id="u-2", username="bob", settings=settings)
...
def test_access_decode_rejects_refresh_token() -> None:
    settings = _settings()
    refresh_token, _ = create_refresh_token(user_id="u-3", username="charlie", settings=settings)
    try:
