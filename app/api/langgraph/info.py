...
async def get_runtime_info(request: Request) -> Any:
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail=f"langgraph_info_timeout: {exc}") from exc
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"langgraph_info_unavailable: {exc}") from exc
