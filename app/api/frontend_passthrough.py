...
async def _request_json(
    ...
    *,
    ...
    response = await client.request(method=method, url=url, headers=_forward_headers(request), json=payload)
...
def _normalize_assistant_item(raw: Any) -> dict[str, Any] | None:
    ...
    return {
        ...
        "graph_id": raw.get("graph_id") if isinstance(raw.get("graph_id"), str) else None,
        ...
        items.append(
            {
                ...
                "text": str(raw.get("content", "")) if isinstance(raw.get("content"), str) else "",
                "tool_call_id": str(raw.get("tool_call_id")) if raw.get("tool_call_id") is not None else None,
...
def _parse_json(raw: str | None, field_name: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"{field_name} is invalid JSON") from exc
    if not isinstance(payload, dict):
        raise HTTPException(status_code=400, detail=f"{field_name} must be a JSON object")
    return payload


async def _create_thread(request: Request, metadata: dict[str, Any] | None = None) -> str:
...
async def create_thread(request: Request, payload: dict[str, Any] | None = None) -> dict[str, Any]:
...
@router.post("/thread")
async def create_or_get_thread(request: Request, payload: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(payload, dict):
...
async def get_history(
    ...
    if not thread_id:
        raise HTTPException(status_code=400, detail="thread_id is required")
    history = await _request_json(request, "POST", f"/threads/{thread_id}/history", payload={"limit": limit})
...
async def get_assistants(
    ...
    if not isinstance(rows, list):
        rows = []
    items = [item for item in (_normalize_assistant_item(raw) for raw in rows) if item is not None]
    return {"items": items}


@router.get("/graphs")
async def get_graphs(request: Request) -> dict[str, list[str]]:
    rows = await _request_json(request, "POST", "/assistants/search", payload={"limit": 500, "offset": 0})
...
async def create_assistant(request: Request, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("name", "description", "config", "context", "metadata", "assistant_id", "if_exists"):
        if key in payload:
...
async def update_assistant(request: Request, assistant_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise HTTPException(status_code=400, detail="payload must be object")
    ...
    for key in ("graph_id", "name", "description", "config", "context", "metadata"):
        if key in payload:
            upstream_payload[key] = payload[key]
    item = await _request_json(request, "PATCH", f"/assistants/{assistant_id}", payload=upstream_payload)
    normalized = _normalize_assistant_item(item)
    if normalized is None:
        raise HTTPException(status_code=502, detail="invalid assistant response")
    return {"item": normalized}
...
async def delete_assistant(request: Request, assistant_id: str, delete_threads: bool = Query(default=False)) -> dict[str, Any]:
...
async def wait_chat(request: Request, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    thread_id = payload.get("thread_id")
    resolved_thread_id = thread_id if isinstance(thread_id, str) and thread_id else await _create_thread(request)
    ...
    result = await _request_json(request, "POST", f"/threads/{resolved_thread_id}/runs/wait", payload=run_payload)
...
async def chat_stream(
    ...
    modes = [item.strip() for item in (stream_mode or "messages,updates,tasks,checkpoints,debug").split(",") if item.strip()]
    ...
    client: httpx.AsyncClient = request.app.state.client
    url = _upstream_url(request.app.state.settings.langgraph_upstream_url, f"/threads/{resolved_thread_id}/runs/stream")
    upstream = await client.send(
        client.build_request("POST", url, headers=_forward_headers(request), json=payload),
        stream=True,
    )
    ...
    if upstream.status_code >= 400:
        ...
        try:
            detail = json.loads(body.decode("utf-8")) if body else {"error": "upstream_error"}
        except Exception:
...
async def langgraph_raw_passthrough(request: Request, full_path: str) -> Response:
    client: httpx.AsyncClient = request.app.state.client
    target = _upstream_url(request.app.state.settings.langgraph_upstream_url, f"/{full_path}")
