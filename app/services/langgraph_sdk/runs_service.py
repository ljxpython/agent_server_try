...
class LangGraphRunsService:
    ...
    async def create(self, thread_id: str, payload: dict[str, Any]) -> Any:
        ...
        create_payload = {key: payload[key] for key in self._CREATE_FIELDS if key in payload}
    ...
    async def wait(self, thread_id: str, payload: dict[str, Any]) -> Any:
        assistant_id = payload["assistant_id"]
        wait_payload = {key: payload[key] for key in self._WAIT_FIELDS if key in payload}
    ...
    async def cancel(self, thread_id: str, run_id: str, payload: dict[str, Any] | None = None) -> Any:
        cancel_payload = {
            key: payload[key] for key in self._CANCEL_FIELDS if payload is not None and key in payload
        }
    ...
    async def cancel_many(self, payload: dict[str, Any] | None = None) -> Any:
        cancel_many_payload = {
            key: payload[key] for key in self._BULK_CANCEL_FIELDS if payload is not None and key in payload
        }
    ...
    async def list(self, thread_id: str, payload: dict[str, Any] | None = None) -> Any:
        list_payload = {key: payload[key] for key in self._LIST_FIELDS if payload is not None and key in payload}
    ...
    async def join_stream(self, thread_id: str, run_id: str, payload: dict[str, Any] | None = None) -> Any:
        join_stream_payload = {
    ...
    async def create_cron(self, payload: dict[str, Any]) -> Any:
        assistant_id = payload["assistant_id"]
        cron_payload = {key: payload[key] for key in self._CRON_CREATE_FIELDS if key in payload}
    ...
    async def search_crons(self, payload: dict[str, Any] | None = None) -> Any:
        search_payload = {
            key: payload[key] for key in self._CRON_SEARCH_FIELDS if payload is not None and key in payload
        }
        return await self._client.crons.search(**search_payload)

    async def count_crons(self, payload: dict[str, Any] | None = None) -> Any:
        count_payload = {
            key: payload[key] for key in self._CRON_COUNT_FIELDS if payload is not None and key in payload
        }
        return await self._client.crons.count(**count_payload)

    async def update_cron(self, cron_id: str, payload: dict[str, Any]) -> Any:
        update_payload = {key: payload[key] for key in self._CRON_UPDATE_FIELDS if key in payload}
    ...
    async def create_cron_for_thread(self, thread_id: str, payload: dict[str, Any]) -> Any:
        assistant_id = payload["assistant_id"]
        cron_payload = {key: payload[key] for key in self._CRON_CREATE_FIELDS if key in payload}
        return await self._client.crons.create_for_thread(thread_id, assistant_id, **cron_payload)
