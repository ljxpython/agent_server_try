...
class LangGraphThreadsService:
    ...
    async def create(self, payload: dict[str, Any] | None = None) -> Any:
        create_payload = {
            key: payload[key] for key in self._CREATE_FIELDS if payload is not None and key in payload
        }
        return await self._client.threads.create(**create_payload)

    async def search(self, payload: dict[str, Any]) -> Any:
        search_payload = {key: payload[key] for key in self._SEARCH_FIELDS if key in payload}
        return await self._client.threads.search(**search_payload)

    async def get_state(self, thread_id: str, payload: dict[str, Any] | None = None) -> Any:
        state_payload = {
            key: payload[key] for key in self._STATE_FIELDS if payload is not None and key in payload
        }
        return await self._client.threads.get_state(thread_id, **state_payload)

    async def get_history(self, thread_id: str, payload: dict[str, Any] | None = None) -> Any:
        history_payload = {
            key: payload[key] for key in self._HISTORY_FIELDS if payload is not None and key in payload
        }
        return await self._client.threads.get_history(thread_id, **history_payload)

    async def count(self, payload: dict[str, Any]) -> Any:
        count_payload = {key: payload[key] for key in self._COUNT_FIELDS if key in payload}
        return await self._client.threads.count(**count_payload)

    async def prune(self, payload: dict[str, Any]) -> Any:
        prune_payload = {key: payload[key] for key in self._PRUNE_FIELDS if key in payload}
    ...
    async def update_state(self, thread_id: str, payload: dict[str, Any]) -> Any:
        update_state_payload = {key: payload[key] for key in self._UPDATE_STATE_FIELDS if key in payload}
    ...
    async def get_state_at_checkpoint(self, thread_id: str, checkpoint_id: str) -> Any:
        return await self._client.threads.get_state(thread_id, checkpoint_id=checkpoint_id)
