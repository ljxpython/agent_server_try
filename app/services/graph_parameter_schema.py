...
class GraphParameterSchemaService:
    ...
    def build_schema(self, graph_id: str) -> dict[str, Any]:
        ...
        context_properties = self._extract_context_properties(runtime_options_file, runtime_context_file)
        metadata_properties = {
    ...
    def _resolve_graph_entry_file(self, graph_config_path: Path, graph_id: str) -> Path | None:
    ...
    def _extract_runtime_config_properties(self, options_file: Path) -> dict[str, dict[str, Any]]:
        ...
        for key, annotation in fields.items():
            if key in {"environment", "model_spec"}:
                continue
            mapped[key] = {"type": self._annotation_to_schema_type(annotation), "required": False}
    ...
    def _extract_context_properties(
        ...
        if context_file.exists():
            try:
                ...
                context_fields = self._extract_dataclass_fields(context_tree, "RuntimeContext")
                for key, annotation in context_fields.items():
                    defaults[key] = {"type": self._annotation_to_schema_type(annotation), "required": False}
            except Exception:
                pass
    ...
    def _extract_dataclass_fields(self, tree: ast.AST, class_name: str) -> dict[str, str]:
        fields: dict[str, str] = {}
        for node in ast.walk(tree):
            ...
            for item in node.body:
                if isinstance(item, ast.AnnAssign) and isinstance(item.target, ast.Name):
                    fields[item.target.id] = ast.unparse(item.annotation) if item.annotation is not None else "Any"
            break
        return fields
