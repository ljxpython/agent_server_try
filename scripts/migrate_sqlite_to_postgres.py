...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source", default="data/archive/agent_platform_v3_migrated_20260307.db")
    parser.add_argument("--database-url", default=None)
    parser.add_argument("--dry-run", action="store_true")
    return parser.parse_args()


def source_rows(source_path: Path, table: str) -> tuple[list[str], list[dict[str, Any]]]:
...
def truncate_tables(engine) -> None:
    statement = "TRUNCATE TABLE " + ", ".join(TABLES_IN_LOAD_ORDER) + " RESTART IDENTITY CASCADE"
    with engine.begin() as conn:
        conn.execute(text(statement))


def insert_rows(engine, table: str, columns: list[str], rows: list[dict[str, Any]]) -> None:
    if not rows:
        return
    ...
    payload = []
    for row in rows:
        payload.append({column: normalize_value(table, column, row.get(column)) for column in columns})
    with engine.begin() as conn:
